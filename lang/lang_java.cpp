...
int watch_judge(pid_t pid) {
    ...
    struct rusage rus;
    max_mem = get_proc_status(pid, "VmRSS:");
    while(1) {
        wait4(pid, &status, 0, &rus);

        tmp_mem = get_proc_status(pid, "VmPeak:");
        if(max_mem < tmp_mem) max_mem = tmp_mem;
    ...
    int t_time_used = rus.ru_utime.tv_sec * 1000 + rus.ru_utime.tv_usec / 1000;
    t_time_used += rus.ru_stime.tv_sec * 1000 + rus.ru_stime.tv_usec / 1000;
    if(time_used < t_time_used) time_used = t_time_used;
